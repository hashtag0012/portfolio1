        ...
        <div className={styles.innerContainer}>
          {newProjects.map((project, index) => {
            ...
            const linkProps = isExternal
              ...
              : { href: project.link, scroll: false };

            const overlayImageSrc = project.overlayImg || project.img;

            return (
              <Link aria-label={`Go ${project.title}`} id={project.id} key={project.id} {...linkProps} className={clsx(styles.card)}>
                <div
                  style={
                    !isMobile
                      ? {
                ...
                >
                  <div className={clsx(styles.container, 'layout-grid-inner')}>
                    ...
                    <div className={styles.imageContainer}>
                      <Image priority src={overlayImageSrc} fill sizes="100%" alt={project.title} />
                    </div>
                  </div>
                </div>
                {/* Background canvas image restored to original backgrounds (project.img) */}
